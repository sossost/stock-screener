...
function generateTableCaption(
  ...
  if (filterState.revenueGrowth) {
    if (filterState.revenueGrowthRate !== null && filterState.revenueGrowthRate !== undefined) {
      parts.push(
        `• 매출 ${filterState.revenueGrowthQuarters ?? 3}분기 연속 상승 + 평균 성장률 ${filterState.revenueGrowthRate}% 이상 종목만`
      );
    } else {
      parts.push(
  ...
  if (filterState.incomeGrowth) {
    if (filterState.incomeGrowthRate !== null && filterState.incomeGrowthRate !== undefined) {
      parts.push(
        `• 수익 ${filterState.incomeGrowthQuarters ?? 3}분기 연속 상승 + 평균 성장률 ${filterState.incomeGrowthRate}% 이상 종목만`
      );
    } else {
      parts.push(
          ...
          <div>
            {tickerSearch ? (
              <>
                ...
                개 / 전체{" "}
                <span className="font-semibold">{totalCount ?? data.length}</span>개
              </>
            ) : (
              <>
                총{" "}
