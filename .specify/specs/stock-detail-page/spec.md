...
**Created**: 2025-11-26  
**Status**: Phase 4 Complete (동종업계 비교 제외)
...
| Phase 4 | 주가 차트 (캔들스틱 + RSI/MACD) | P4 ✅ |
| Phase 5 | 동종업계 비교 (추후 고도화) | P5 |
...
사용자가 캔들스틱 차트와 기술적 지표(RSI, MACD)를 시각적으로 확인할 수 있다.

**Why this priority**: 기술적 분석의 핵심이지만, 차트 라이브러리 선정 및 구현 복잡도가 높아 후순위.

**Reference**: https://finviz.com/quote.ashx?t=AAPL

**Acceptance Scenarios**:

1. **Given** 일별 가격 데이터가 있는 종목, **When** 상세 페이지 로드, **Then** 캔들스틱 차트가 OHLC 데이터로 표시된다.
2. **Given** 캔들스틱 차트, **When** 렌더링, **Then** SMA 20(초록)/50(주황)/100(분홍)/200(하늘) 이동평균선이 오버레이된다.
3. **Given** 캔들스틱 차트 하단, **When** 렌더링, **Then** 거래량 바 차트가 표시된다.
4. **Given** RSI 패널, **When** 렌더링, **Then** 14일 RSI 값이 70/30 기준선과 함께 표시된다.
5. **Given** MACD 패널, **When** 렌더링, **Then** MACD(12,26), Signal(9), 히스토그램이 표시된다.
6. **Given** 차트 기간 선택(1M/3M/6M/1Y), **When** 기간 변경, **Then** 해당 기간의 데이터만 차트에 표시된다.
...
#### Phase 4
- **FR-014**: 캔들스틱 차트를 표시해야 한다 (Lightweight Charts 사용).
- **FR-015**: 차트에 SMA 20/50/100/200 이동평균선을 오버레이해야 한다.
- **FR-016**: 차트 하단에 거래량 바 차트를 표시해야 한다.
- **FR-017**: RSI(14일) 지표를 별도 패널에 표시해야 한다 (70/30 기준선 포함).
- **FR-018**: MACD(12,26,9) 지표를 별도 패널에 표시해야 한다 (Signal Line + 히스토그램).
- **FR-019**: 차트 기간 선택 (1M/3M/6M/1Y) 기능을 제공해야 한다.
- **FR-020**: 동일 섹터 종목 비교 테이블을 제공해야 한다.
...
### Phase 4
- **SC-010**: 캔들스틱 차트가 OHLC 데이터로 정상 렌더링된다.
- **SC-011**: SMA 20/50/100/200 이동평균선이 색상 구분되어 표시된다.
- **SC-012**: RSI 지표가 14일 기준으로 정확하게 계산되어 표시된다.
- **SC-013**: MACD 지표가 12/26/9 기준으로 정확하게 계산되어 표시된다.
- **SC-014**: 기간 선택(1M/3M/6M/1Y) 시 해당 기간의 데이터만 표시된다.
- **SC-015**: 동종업계 비교에서 같은 섹터 종목만 표시된다.
...
3. ~~주가 차트 라이브러리 선정~~ → **결정됨: Lightweight Charts** (TradingView 오픈소스)
4. 동종업계 비교 시 표시할 종목 수와 정렬 기준?
5. 외부 링크(Seeking Alpha, Yahoo Finance 등) 포함 여부?
6. RSI/MACD 계산은 클라이언트에서 수행 (DB 저장 불필요)
